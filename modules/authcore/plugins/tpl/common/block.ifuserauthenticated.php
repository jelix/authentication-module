...
/**
 * a special if block to test easily if the current user is authenticated
 *
 * <pre>{ifuserauthenticated} ..here generated content if the user is authenticated  {/ifuserauthenticated}</pre>
 *
 ...
 */
function jtpl_block_common_ifuserauthenticated($compiler, $begin, $params=array())
{
    if($begin){
        if(count($params)){
            $content='';
