Configuration of the ldap backend
==================================

If you used the ldapdao or multiauth module, you already know how to configure 
the ldap plugin for authloginpass: they have almost the same configuration parameters.

Connection configuration
------------------------

You should create an ldap profile, with the name indicated into the `profile`
parameter of the backend configuration. 
The ldap profile should be into the `var/config/profiles.ini.php` file.

For example, if the profile is named `myldap` (like in the following example), 
so you should set `profile=myldap`.
